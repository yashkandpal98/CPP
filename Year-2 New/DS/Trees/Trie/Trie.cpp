...
void add(trptr &T, string s, int i)
{
    ...
    else
    {
        if(T->tag == 0)                                                             //If data node is found
        {
            ...
            int j = 0;
            while(T->key.word[j])                                                   //Pushing existing data onto a string
            {
                s1 += T->key.word[j];
                j++;
            }
...
void sortOrder(trptr T)
{
    if(T != NULL)
    {
        ...
        {
            if(T->key.ptr[26] != NULL) cout<<T->key.ptr[26]->key.word<<" ";
            for(int i = 0; i < 26; i++) sortOrder(T->key.ptr[i]);
        }
    }
}
...
void pattern(trptr T, string s, int i)
{
    if(s[i])
    {
        if(T == NULL) cout<<"\nTree empty.\n\n";
        else
        {
            if(! s[i+1])
            {
                if(T->key.ptr[int(s[i])-97] != NULL)
                {   cout<<"Possible matches: \n";
                    sortOrder(T->key.ptr[int(s[i])-97]);
                }
                else cout<<"\nNo possible match.\n\n";
            }
            else pattern(T->key.ptr[int(s[i])-97], s, i+1);
        }
    }
}
...
int main()
{
    ...
    cout<<endl<<endl<<"Enter string: "; cin>>s;
