...
bdptr newNode(bdptr &T, int &x)         //Creates new node, value to be inserted in parent, and old node
{
    bdptr T1 = new bdnode;
    T1->size = d;
    T1->parent = NULL;
...
void nodeSplit(bdptr &T, int x, bdptr ptr)          //Passing new node, old node and value to be put in parent
{
    if(T->parent == NULL)
    {
...
void create(bdptr &T, bdptr parent, int x)          //Create function
{
    if(T == NULL)
    {
        T = new bdnode;
...
void printCurrentLevel(bdptr T, int level)
{
    if(T != NULL)
    {
        ...
        {
        	for(int i = 0; i <= T->size; i++) printCurrentLevel(T->ptr[i], level-1);
		}
	}
}
